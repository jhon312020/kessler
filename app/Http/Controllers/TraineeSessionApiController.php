...
class TraineeSessionApiController extends Controller
{
    ...
    public function index(Request $request) {
      $validator = [];
      $result = array('success'=> 0, 'message'=>'Invalid Request!');
       if ($request->isMethod('post')) {
        ...
        if (!$validator->fails()) {
          ...
          if ($record) {
            ...
            $record->api_token = \Str::random(60);
            $record->session_start_time = json_encode($sessionStartTime);
            $record->save(); 
            if ($record['session_number'] <= 4) {
              return redirect('sessions/'.$record->api_token);
            } else {
              return redirect('write');
            }
          } else {
             $result['success'] = false;
             $result['message'] = 'INVALID PIN! Please contact your trainer..';
          } 
        } 
      } 
